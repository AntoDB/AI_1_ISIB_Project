...
def random_search(graph, start, goal, transfers):
        ...
        if current_station not in visited:
            ...
            for neighbor, (travel_time, lines) in graph[current_station].items():
                for line in lines:
                    ...
                    # Ajouter les nouveaux chemins, le coût total et la ligne à la file dans des positions aléatoires
                    queue.insert(random.randint(0, len(queue)), (new_path, new_cost, line))
...
path, total_cost = random_search(metro_network, start_station, goal_station, line_changes)
