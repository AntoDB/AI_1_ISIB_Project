    ...
    while queue:
        ...
        for neighbor in graph[current_station]:
            if heuristics[neighbor] == float('inf'):  # Non visité
                ...
                # Si la station est loin expret pour distingué BFS (comme Simonis & Porte de Namur), on augmente l'heuristique :
                if graph[current_station][neighbor][0] > 100:
                    heuristics[neighbor] += graph[current_station][neighbor][0] - 10 # Rajout de la distance mise et sous-estimé

                queue.append((neighbor, distance + 1))
    
    return heuristics

def greedy_search(graph, start, goal, transfers):
    # Priority queue (file de priorité) pour les chemins à explorer, triée par coût heuristique estimé
    queue = [(0, [start], 0, None)]  # (heuristic cost, path, current cost, current line)
    heapq.heapify(queue)  # Convertir une liste en un heap (tas)
    visited = set()  # Ensemble pour garder une trace des stations visitées
    all_heuristics = calculate_heuristics(graph, goal) # Demande a calculer toutes les heuristiques des stations

    while queue:
        # Récupération et suppression du chemin avec le coût heuristique le plus bas de la file de priorité
        ...
        # Si la station actuelle n'a pas encore été visitée
        if current_station not in visited:
            ...
            for neighbor, (travel_time, lines) in graph[current_station].items():
                for line in lines:
                    ...
                    # Calculer le coût heuristique estimé pour atteindre l'objectif depuis la station voisine (nombre de station depuis le goal jusqu'à la courante)
                    heuristic_cost = all_heuristics[neighbor]

                    # Ajouter les nouveaux chemins avec le coût heuristique à la file de priorité
...
path, total_cost = greedy_search(metro_network, start_station, goal_station, line_changes)
