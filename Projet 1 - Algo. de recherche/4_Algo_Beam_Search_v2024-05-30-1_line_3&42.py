...
def beam_search(graph, start, goal, transfers, beam_width):
    # File de priorité pour les chemins à explorer, triée par coût heuristique estimé
    ...
    while queue:
        ...
        # Explorer tous les chemins de la profondeur actuelle
        while queue:
            ...
            if current_station not in visited:
                ...
                for neighbor, (travel_time, lines) in graph[current_station].items():
                    for line in lines:
                        ...
                        # Calculer le coût heuristique estimé pour atteindre l'objectif depuis la station voisine
                        heuristic_cost = all_heuristics[neighbor]

                        # Ajouter le nouveau chemin à la file de la profondeur suivante
                        next_queue.append((heuristic_cost, new_path, new_cost, line))
                        print(f'    New path: {new_path}, Heuristic_cost {heuristic_cost}')
                print(f'New queue: {queue}')

        # Restreindre la queue à la largeur de faisceau spécifiée
        queue = heapq.nsmallest(beam_width, next_queue)
        print(f'--- Restricted queue: {queue}')
        print()
...
beam_width = 2
path, total_cost = beam_search(metro_network, start_station, goal_station, line_changes, beam_width)
